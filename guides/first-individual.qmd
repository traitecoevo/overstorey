...
This guide walks through modelling the dynamics of a single individual: how to
construct one, compute its rates, grow it through time and to a target size,
and compare growth trajectories across traits and light environments. It also
reproduces the analysis behind Fig. 2 of the manuscript. Throughout we use the
[`FF16`](../theory/models/ff16.qmd) strategy, describing the Falster-FitzJohn
model of light competition in
forest stands; however an equivalent `Individual` component of the model is
present in all strategies. For the differential equations that underpin these
dynamics, see [Size-structured PDE](../theory/size-structured-pde.qmd).
...
The helper `fixed_environment` defined below creates an
...
the individual — its leaf-level carbon budget, described in
[Assimilation & hydraulics](../theory/assimilation-hydraulics.qmd) — and from
that its demographic rates:
...
<!-- TODO(#481): next bit is a bit vague -->
...
Then we can use the `params` object to set a low LMA ("fast growth") and a high LMA ("low growth") species:
...
The compensation point and growth curve also vary with plant size.
Compare a seedling with a plant at half its maximum height (`hmat / 2`),
using a small helper `g()` that evaluates `f()` across the openness
sequence:

```{r}
#| label: fig-compensation-size
#| fig-cap: "Height growth rate against canopy openness for a seedling and an adult."
...
#| fig-cap: "Height growth rate against canopy openness for low-LMA (solid) and high-LMA (dotted) species, as seedling and adult."
...
Note that the high-LMA species (dotted), which has the lower growth
rate at most light levels as a seedling, achieves the higher growth
rate across all light levels as an adult.
</content>
</invoke>

...
the [size-structured model](../theory/size-structured-pde.qmd), then summarise,
integrate, and interpolate the results.

::: {.callout-note}
## Running the `TF24` model needs a configured soil-water environment
The [`TF24`](../theory/models/tf24.qmd) strategy couples plant growth to a multi-layer soil-water balance,
...
run_mypatch <- function(
  ...
  p1 <- add_strategies(p0, traits, hyper_par_fn,  birth_rate = birth_rate_list)
...
The [node-spacing algorithm](../theory/implementation/node-spacing.qmd) determines the optimum introduction times for the [characteristics](../theory/implementation/characteristic-method.qmd), which means that the time values associated with steps may not correspond to known times that we might like to extract information about. For example, what the total amount of biomass per $m^2$ will be at 50 years. By doing this, it also means that we can integrate between defined times. Let's work with our results again by running a patch again and then tidying.

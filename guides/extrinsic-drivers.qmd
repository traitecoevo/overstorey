...
Rainfall is handled inside `TF24_Environment`, and is set upon construction. If nothing is passed to the `Environment()` constructor, rainfall
...
Per species birth rates are set via the `add_strategies()` function. Each object in the list is either a constant number or an embedded list with `x` and `y` control points. There must be the same number of items in the list as there are species:

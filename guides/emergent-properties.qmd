...
# Light extinction coefficient used when weighting leaf area into competition.
k_I <- patch$strategies[[1]]$pars$k_I
```

# Node densities

There was a bit of hassle in the [patch dynamics guide](patch-dynamics.qmd)
...
As with height in the [patch dynamics guide](patch-dynamics.qmd),
node density is a matrix of time and node identity.  However, to show the
profiles for a given time slice using `matplot` it is convenient to have each
column be the state of the system at a given time, so here each row is a node
...
traveling the [characteristic equations](../theory/implementation/characteristic-method.qmd)
of the SCM).  Later on (20
...
from; a profile of leaf area with respect to height.  The collected
output records the leaf area (`competition_effect`) of an individual
in each node, which we weight by node density and the light
extinction coefficient `k_I` (see [Canopy & light-environment models](../theory/implementation/canopy-models.qmd))
to get each node's contribution to patch competition:
...
population.  This differs from the profile in the [first individual guide](first-individual.qmd)
...
The collected output records node state rather than instantaneous
rates, so here we approximate each node's height growth rate by finite
differences along its trajectory (the change in height between
...
(for formula see the [size-structured PDE](../theory/size-structured-pde.qmd) page).
To achieve this we first need

---
title: "Prototyping a soil water-balance bucket model"
description: "An early deSolve prototype exploring soil water dynamics — infiltration, drainage, diffusivity flux, and Penman-Monteith evaporation."
date: 2024-05-22
categories: [experimental, soil-water]
reading-time: true
---
...
::: {.callout-note}
This is an exploratory experiment from 2021, kept for the record. It prototypes
soil water dynamics directly with `deSolve` and does not use the `plant` package
API — it is not maintained and may not reflect the current model.
:::

*Originally authored by Isaac Towers, 2021-05-19.*
...
$\theta_{sat}$ Soil moisture capacity $(m^3 m^{-3})$
...
The same idea, now wired up as a general multi-layer model with configurable
monitored depths, runoff and deep-drainage bookkeeping, and a seasonal
rainfall signal.

```{r}
#| label: multilayer-params
...
A hard-coded ten-layer expansion of the same scheme, kept here as scratch
working.

```{r}
#| label: tenlayer-params
...
```{r}
#| label: diffusivity-3layer-params
# Timestep
t <- seq(0,100, by = 0.1)
...
```{r}
#| label: diffusivity-3layer-solve
logistic_solution <-
  map_dfr(
    params,
    ~ode(
...
```{r}
#| label: diffusivity-3layer-plot-1
logistic_solution %>%
...
```{r}
#| label: diffusivity-3layer-plot-2
logistic_solution %>%
...
```{r}
#| label: diffusivity-3layer-plot-3
logistic_solution %>%
...
```{r}
#| label: infiltration-drainage-params
# Timestep
t <- seq(0,10, by = 0.01)
...
```{r}
#| label: infiltration-drainage-solve
logistic_solution <-
  map_dfr(
    params,
    ~ode(
...
```{r}
#| label: infiltration-drainage-plot
logistic_solution %>%
  ggplot(aes(time, theta, col=ks)) +
  geom_line()
```
...
```{r}
#| label: evap-lai-variable-params
# Timestep
t <- seq(0,10, by = 0.01)

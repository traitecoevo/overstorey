...
- `vcmax`: the maximum rate carboxylation ($\mu mol~m^{-2}~s^{-1}$)
...
```{r}
leaf$set_physiology(PPFD = 1000, psi_soil = 0, leaf_specific_conductance_max = 4.8e-05, atm_vpd = 2, ca = 40, sapwood_volume_per_leaf_area = 1.8e-04, rho = 608, a_bio = 0.0245, leaf_temp = 25, atm_o2_kpa = 21, atm_kpa = 101.3)
```
...
tibble(C_i = C_i) %>%
  ...
  mutate(A_net_plus_R_d = A_net + leaf$R_d_) %>%
  pivot_longer(cols = c(A_j, A_c,A_net, A_net_plus_R_d), names_to = "rate", values_to = "assim") %>%
  ggplot(aes(x = C_i, y = assim))+
...
$$E_{supply} = \int^{\psi_l}_{\psi_s}k_l(\psi)\delta\psi $$ {#eq-e-supply}
...
$$H(\psi_l) = b_1z(1-e^{-{(\frac{\psi_l}{b})}^c})^{b_2}.$$ {#eq-hydraulic-cost}
...
```{r}
f <- function(psi_stem){
  leaf$profit_psi_stem_TF(psi_stem)
...
while (abs(bound_b - bound_a) > delta_crit) {
  ...
  profit_at_c = leaf$profit_psi_stem_TF(bound_c);
  profit_at_d = leaf$profit_psi_stem_TF(bound_d);
...
(profit = leaf$profit_psi_stem_TF(opt_psi_stem))

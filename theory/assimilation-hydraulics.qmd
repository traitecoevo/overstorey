...
::: {.callout-note}
## Prerequisites
This page builds on [the FF16 model](models/ff16.qmd) and [The maths, gently](maths-primer.qmd). Read those first if the notation here is unfamiliar.
:::

## The idea first

A leaf makes its living by opening tiny pores, called stomata, to let carbon
dioxide in from the air. Every time it does this, water vapour escapes the
same way. That escaping water has to be replaced, and pulling replacement
water up from the soil through the plant's internal plumbing has a cost: it
draws the water column in the stem tighter, and if pulled too hard, that
plumbing can fail. So a leaf faces a trade-off. Opening the pores wider lets
in more carbon dioxide, which means more sugar produced through
photosynthesis, but it also draws more water, straining the plumbing and
raising the risk of damage further. Opening them less protects the plumbing
but starves the leaf of carbon dioxide, so it makes less sugar. Somewhere
between "shut" and "wide open" sits a setting that gets the most net benefit:
the sugar gained minus the cost of running and protecting the water supply.
This page treats the leaf as if it always finds that best setting, a
convenient shorthand for describing how carbon gain responds to sun, air
dryness, and water availability. The rest of this page makes this precise.

::: {.callout-warning}
## Code not executed — needs updating to the current `Leaf()` API
The worked examples on this page were written against an earlier `Leaf()`
constructor. In the installed `plant` ({{< var plant.version >}}) the leaf model gained an
explicit root-hydraulics parameter set, so the seed call
`Leaf(vcmax = ..., ...)` now fails: `vcmax` is renamed `vcmax_25`, and
`root_c`, `root_b`, `root_psi_crit` (plus `beta2`, `jmax_25`, `hk_s`, …) are
required with no defaults. Because every figure below derives from that one
`leaf` object, execution is disabled page-wide (`eval: false`) until the call
is updated with the correct current parameter values — these are model values
and should be supplied by the model authors, not guessed. The maths and the
code listing remain accurate as documentation.
:::

This leaf-level carbon/water economy is the heart of the
[TF24 strategy](models/tf24.qmd): it determines how a unit of leaf area
assimilates carbon and transpires water in response to the surrounding
environment. The model maximises *profit* — photosynthetic revenue minus the
hydraulic cost of supplying transpired water — by optimising the leaf xylem
water potential.
...
- `c`: Shape parameter for the hydraulic vulnerability curve (unitless)
- `b`: Sensitivity parameter for the hydraulic vulnerability curve (-MPa)
- `psi_crit`: $\psi_l$ at which 95% of conductivity is lost. Maximum water potential. (-MPa)
...
through the compiled `TF24` C++ `Leaf` object and checks the result matches the
manual golden-section search above. It is disabled here (`eval: false`) because
it is a self-consistency check on the implementation rather than an illustration
of the theory.

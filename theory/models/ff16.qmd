...
Every plant carries a set of traits -- things like how it builds leaves, how
dense its wood is, how it allocates mass to different tissues -- and a
current size. FF16 takes those two ingredients, together with the light
available to the plant, and turns them into rates: how fast it grows taller,
how fast it produces seeds, and how likely it is to die. The engine
underneath all three rates is the same: the plant captures carbon through
photosynthesis, spends some of that income on maintaining and replacing its
existing tissues, and what's left over is net production it can invest in
new growth. A larger plant with more leaf area can capture more light, but
it also carries more tissue to maintain, so the model tracks how that
balance shifts as the plant gets bigger. Traits shape the balance too: a
plant built for fast height growth channels production differently than one
built to survive shade or drought. Once you know how much a plant produces
and how it chooses to allocate that production, growth, reproduction, and
mortality all follow as natural consequences of the same underlying
physiology, rather than as separate, disconnected assumptions.
...
which in turn is calculated based on well-understood physiology (see @fig-phys). Various physiological parameters
influence demographic outcomes. Varying these parameters allows accounting for
species differences, potentially via traits. Tables summarize the units and definitions of all variables, parameters, and hyper-parameters used in
the material below.
...
where $q(z, H)$ is the vertical distribution of leaf area with respect to height $z$ (@eq-crown3).
...
of net biomass production (@eq-dbdt) that is allocated to

...
None of that is unusual. What is worth writing down is *why each step is there*, because each one is doing a specific job, and when a step is skipped it is usually because someone has forgotten which job it was doing. This page is the reasoning; the mechanical detail (label names, board fields, commit-message shape) lives in [`plant-meta/governance/`](https://github.com/traitecoevo/plant-meta/tree/main/governance) and is linked from each section rather than repeated here.
...
The model is [Gitflow](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow), lightly applied. There is a default branch that integrates work, a release branch where a release branch exists, and a short-lived branch per change.

**Check the default branch before you branch — they differ across the family.** `plant` integrates on `develop` and releases from `master`; most of the rest are single-branch. As currently configured:
...
We are a small team with experienced developers, and review is not enforced by branch protection. Anyone who judges a change small, urgent, or obvious enough can merge their own PR. That is a deliberate choice, not an oversight — a hard approval gate on a two-person repo mostly teaches people to route around it.

But note that **even with the ability to skip review entirely, most of us still work through PRs**, and that is the part worth internalising. The value is not the approving click. It is that opening a PR forces you to look at your own diff as a diff — separated from the reasoning that produced it, in the order a stranger will read it — and that alone catches a surprising amount. Self-review is the floor, not a formality. If nobody else is going to read it, read it yourself, properly, before you merge.
